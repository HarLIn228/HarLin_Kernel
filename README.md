...
### 中断与调度
- IDT 中断描述符表、PIC 与 Local APIC 中断控制器
- 优先级调度、时间片轮转、就绪/睡眠队列
...
- ATA PIO LBA28：磁盘读写
- RTL8139：PCI 网卡驱动、USB UHCI 控制传输框架
- Sound Blaster 16：DMA 方式 16-bit PCM 播放、WAV 解析
- 鼠标驱动、RTC 时钟、PCI 总线枚举
...
- VESA 图形模式（800x600x24/32）、8x16 字体渲染
- 内置图形 Shell（命令：help、about、clear、info、beep、halt）

### 电源管理
- ACPI：RSDP 扫描、FADT 解析、DSDT S5 解析
- acpi_power_off() 硬件关机、acpi_reboot() 系统重启

### 引导
- BIOS 阶段分段引导（boot.asm + stage2.asm）
- El Torito CD-ROM 启动、INT 0x15 E820 内存探测
- NUMA 节点拓扑发现（SRAT/SLIT 解析）

## 文档

- `docs/手册.md`：小白上手指册
- `docs/架构.md`：内核架构说明（含 9 张架构图）
