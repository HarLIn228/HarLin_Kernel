...
`src/harlin/proc/scheduler.c`、`src/harlin/proc/exec.c`、`src/harlin/proc/wait.c`：

- `Harlin_Fork(parent_pid, out_child_pid)`：在 `processes[]` 中找空闲 slot，复制父进程描述符（rip / rsp / pml4 / stack / handles / 优先级 / fair_key），子进程状态设为 READY
- `Harlin_ExecFromPath` / `Harlin_ExecFromBuffer`：从 mem_fs 或内存缓冲区读取 elf，调底层 `process_create_elf` 加载为新 slot
- `Harlin_NotifyExit` + `Harlin_Wait` + `Harlin_WaitAny`：独立退出码表，登记 / 等待 / 任意子退出；reap 机制保证退出码可被一次性回收

### 6.6 事件通知

`src/head/proc/event.h` 与 `src/harlin/proc/event.c`：

- `Harlin_NotifySend(to_pid, event)` / `Harlin_NotifyWait(pid, &out_event)`
- 进程事件队列（默认 16 槽）投递 / 消费；满则丢弃
...
`src/head/fs/path_walk.h` 与 `src/harlin/fs/path_walk.c` 实现：

- `Harlin_PathParse`：按 `/` 拆分路径为 `struct path_parts` 段数组，写入 `buf` 内部存储
- `Harlin_PathNormalize`：在 `Harlin_PathParse` 之上用栈式处理 `.` / `..`，输出规范化路径
- `is_absolute` 标志记录原始路径是否以 `/` 开头

### 7.5 内存虚拟文件系统 (mem_fs)

`src/head/fs/mem_fs.h` 与 `src/harlin/fs/mem_fs.c` 实现：

- 节点池 `struct mem_fs_node[64]`，每节点含 `name / first_child / next_sibling / parent / mode / owner / group / payload / payload_size`
- 路径解析复用 `Harlin_PathNormalize` 后逐段下降
- API：`Harlin_MemFsInit` / `Harlin_MemFsMkdir` / `Harlin_MemFsRmdir` / `Harlin_MemFsCreate` / `Harlin_MemFsRemove` / `Harlin_MemFsWrite` / `Harlin_MemFsRead` / `Harlin_MemFsLs` / `Harlin_MemFsLookupNode`

### 7.6 文件权限位

`src/head/fs/perm.h` 与 `src/harlin/fs/perm.c`：

- 9 位 rwx：`PERM_OWN_R/W/X`（bit 8-6）、`PERM_GRP_R/W/X`（bit 5-3）、`PERM_OTH_R/W/X`（bit 2-0）
- `Harlin_PermCheck(mode, file_owner, file_group, uid, gid, want)`：先匹配 `uid==file_owner`（用 owner 位）、再 `gid==file_group`（用 group 位）、其余走 other 位，按 `want` 校验

### 7.7 COW 文件系统

`src/head/fs/cow_fs.h` 与 `src/harlin/fs/cow_fs.c` 在 mem_fs 之上叠加版本管理：

- `struct cow_version[16]` 版本池，按 `file_node` 关联到 mem_fs 文件
- 版本号由 `ver_counter` 全局计数生成 `v000`~`v999` 字符串
- `Harlin_CowSnapshot`：复制当前文件 payload 生成新版本
- `Harlin_CowWrite(expected_ver)`：条件写，期望版本不匹配返回 `HARLIN_BUSY`
- `Harlin_CowReadAt(ver, offset)`：按版本 + 偏移读取
- `Harlin_CowFork`：跨文件节点克隆所有版本

### 7.8 /proc 虚拟文件系统

`src/head/fs/proc_fs.h` 与 `src/harlin/fs/proc_fs.c`：

- 不在 mem_fs 中预创建节点，每次 `Harlin_ProcFsRead` 动态拼出文本
- `/proc/uptime` 输出 `uptime_ticks=<N>`
- `/proc/meminfo` 输出 `free=<N> used=<N>`（来自 `Harlin_PmmTotalPages` + 0 占位）
- `/proc/cpuinfo` 输出 `cpus=<N>`（来自 `Harlin_SmpCpuCount`）
- `/proc/loadavg` 输出 `load=0.0` 占位
...
| 音频 | PC Speaker | `src/asm/drv/speaker.asm` |
...
│   ├── net/                #   网络头文件 (network.h)
│   └── proc/               #   进程头文件 (scheduler.h, ipc.h, pipe.h, elf.h)
├── tools/                  # 辅助工具
